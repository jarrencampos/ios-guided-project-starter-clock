...
class ClockView: UIView {
    ...
    private var digitFont: UIFont {
        return UIFont.systemFont(ofSize: 15.0 + frame.size.width / 50.0)
    }
...
//             numerals
            ...
            context.strokePath()
            // second's center
            let secDotRadius: CGFloat = 3.0
            let secRect = CGRect(x: clockCenter.x - secDotRadius,
                                 y: clockCenter.y - secDotRadius,
                                 width: secDotRadius * 2,
                                 height: secDotRadius * 2)
            
            context.addEllipse(in: secRect)
